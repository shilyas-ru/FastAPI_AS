...
@app.post("/hotel")
def create_hotel(hotel_title: str = Body(embed=True),
...
def hotel_put(hotel_id: int = Body(embed=True),
              hotel_title: str = Body(embed=True),
              hotel_name: str = Body(embed=True)
              ):
    """
...
@app.patch("/hotel",
           ...
           description="<h1>Пояснение по hotel_patch</h2>"
                       ...
                       "<ul>Параметры (передаются методом Body):"
...
def hotel_patch(hotel_id: int = Body(embed=True),
                hotel_title: str | None = Body(embed=True, default=None),
                hotel_name: str | None = Body(embed=True, default=None)
                ):
    """
    Обновление каких-либо данных выборочно или всех данных сразу
